  ...
  <div class="comments">
    <div class="comment_hd">
      ...
      <div class="userInfo" v-if="userInfo">
        ...
        <el-image :src="userInfo.avatarUrl" class="avatar" lazy />
      </div>
    </div>
    <el-form class="comment_box">
      ...
      <div class="comment_box_footer">
        <el-button type="primary" color="#74cdff" @click="subComment">评论</el-button>
    ...
    <div class="comment_area">
      <template v-if="comments.length">
        <div class="comment_item" v-for="(item, index) in comments" :key="item.commentId + index + ''">
          ...
          <div class="comment_info">
            ...
            <div class="comment_reply" v-for="replyItem in item.beReplied" :key="replyItem.beRepliedCommentId">
              <router-link :to="{ path: '/user', query: { id: replyItem.user.userId } }">{{ replyItem.user.nickname }}</router-link
              >: {{ replyItem.content }}
            </div>
            <div class="comment_footer">
              <div class="comment_time">{{ $utils.formatMsgTime(item.time) }}</div>
              <div class="comment_oper">
                <em class="comment_del" v-if="userInfo && userInfo.userId === item.user.userId"><i class="iconfont icon-del"></i></em>
                <span :class="[item.liked ? 'active' : '']" @click="likeComment(item)"><i class="iconfont icon-praise"></i>({{ item.likedCount }})</span>
                <span class="replyComment" @click="replyComment(item, index)"><i class="iconfont icon-jianyi"></i></span>
              </div>
            </div>
        ...
        <div class="pagination">
          <el-pagination @current-change="currentChange" layout="prev, pager, next" :total="total"> </el-pagination>
        </div>
      </template>
...
import ReplyComment from "@/components/Reply";
import { getCurrentInstance, computed, onMounted, watch, reactive, toRefs } from "vue";
import { reqComment, reqCommentVideo, reqCommentSong, reqCommentMv, reqAlbumComment, reqCommentLike } from "@/api";
export default {
  name: "Comments",
  components: {
    ReplyComment,
  ...
  setup(props) {
    ...
    const getComment = () => {
      ...
      switch (props.type) {
        case 0:
          getCommentList(reqCommentSong);
          break;
        case 1:
          getCommentList(reqCommentMv);
          break;
        case 3:
          getCommentList(reqAlbumComment);
          break;
        case 5:
          getCommentList(reqCommentVideo);
          break;
      }
    };
    ...
    const getCommentList = async type => {
      ...
      const result = await type({ id: info.curId, limit: info.limit, offset: info.offset, before: info.before, timestamp: new Date().valueOf() });
    ...
    const commentHandler = async (t, content, commentId) => {
      ...
      const result = await reqComment(params);
    ...
    const likeComment = async item => {
      ...
      const { data: res } = await reqCommentLike({ id: info.curId, cid: item.commentId, t: Number(!item.liked), type: props.type });
